...
"##;
    ...
    unsafe {
        analyzer_result.output(&output_config, &mut buffer.as_mut_vec()).expect("Output failed")
    }

    asserting("Output is correct").that(&buffer.as_ref()).is_equal_to(expected_output);
}
